...
**先告诉你结论：`watch.py` 自己会补，通常不用你管。**

以前 `notify_all()` 发送失败只打一行 `[error]`，而状态**已经落盘** →
那条开播通知就**永久丢失**（2026-09-23 实盘真丢过一条）。
现在失败的通知会连正文一起记进 `state_<room_id>.json` 的 `notify` 字段，
之后每一轮都自动重发，间隔按 1、2、4、8… 分钟退避，
直到送出去、或到达上限（默认 30 次 / 6 小时）。

日志里怎么看：

| 日志行 | 含义 |
|---|---|
| `[error] onebot 通知失败：...` | 这一轮没送出去，**已记下**，后面会自动补 |
| `[warn] 「开播」通知待补发：退避中，还要等 N 秒` | 在等退避，正常 |
| `[ok] 「开播」通知补发成功（第 N 次尝试）` | 补上了 |
| `[error] 「开播」通知已重试 30 次仍未送出，放弃自动重试` | 到顶了，**需要人工处理** |

**什么时候还需要手动补**：只有「自动重试也到顶了」这一种情况
（比如 NapCat 挂了超过 6 小时）。前提是**主播现在还在播**：
...
> 顺带一提：`--recover-notify` 也不会和新的自动重试打架 ——
> 它把 `is_live` 改回 `false` 后，卡住的那条旧记录会因为「状态已翻面」被自动作废，
> 然后走正常流程重新发一条。
...
#### 发失败会自动重试（v1.0.0 修，之前会永久丢消息）

报平安走的是**独立于 NapCat 的网络通道**（pushplus / Server酱 …），
这条通道也会抽风。2026-09-23 和 09-24 的 10:00 就各丢了一条：
那两轮体检恰好卡满 `HTTP_TIMEOUT`（10 秒）后失败，而当时的代码不看发送结果、
照样把当天标记成「已报」—— 于是当天再也不会重试，消息永久消失，
日志里只留一句 `送达：无`，连原因都查不出来。

现在：

- **只有真送到至少一条通道，才写 `last_daily_ok`**（送达了才落章）；
- 失败则记进状态文件的 `daily_ok_retry`，按 **3 → 6 → 12 → 24 → 30 分钟** 退避重试；
- 试满 `DAILY_OK_RETRY_MAX` 次仍失败才放弃，并在报告里写明
  「今天你会收不到报平安，别把它当成机器挂了」；
- 跨天自动重置，昨天的放弃不影响今天；
- 失败原因（哪个通道、报了什么错）**一并写进 `alerts.log`**。

两个可调项：

```bash
DAILY_OK_RETRY_MAX=5                    # 含首次在内最多试几次
DAILY_OK_RETRY_BACKOFF_MINUTES=3        # 首次等 3 分钟，之后翻倍，最长 30 分钟
```

再也不用猜「今天到底发没发」——`--status` 里直接有答案：

```
  报平安 : 今天已送达（10:00 后发的那条）
  报平安 : 到点了，下一轮就发
  报平安 : 重试中：已试 2 次，最近一次 2026-09-24 10:06:12
  报平安 : ⚠️ 今天已放弃（试了 5 次都没送出去，今天不会再发）
  报平安 : 还没到点（10:00 之后才发）
```

> 另外：`HTTP_TIMEOUT` 现在**对告警 webhook 也生效**（以前 webhook 写死 10 秒，
> 改配置也没用）。经常碰上通道慢的话，把它调到 20。
