...
- 内存判定为「**不足**」→ **停下**，把输出发回给用户等指示（需要先加 swap 或换方案）。
  NapCat 是 Electron 应用，常驻 400MB~1GB，小内存机器会被 OOM killer 打死。
- 提示 `/opt/napcat` 或 `/opt/douyu-live-notify` **已被占用** → **停下**报告，别覆盖别人的目录
- 判定「充足」或「勉强」，且目标目录干净 → 继续
...
**这一步你无法完成。** 请把下面这段话原样交给用户，然后**停止执行，等确认**：

```
请在你自己的电脑上开一个 SSH 隧道（保持这个终端窗口开着）：
    ssh -N -L 6099:127.0.0.1:6099 root@<这台服务器的公网IP>

然后浏览器打开：http://127.0.0.1:6099/webui
登录密码在服务器上的 /opt/napcat/.env 里，字段名是 WEBUI_TOKEN

进去后用手机 QQ 扫码，登录你准备用来发消息的那个小号。
登录完成后告诉我，我再继续下一步。
```
...
2. 阶段 3 的 `ss -lntp | grep 3000` 结果 —— 确认只绑了 `127.0.0.1`
3. 阶段 7 第 ④ 关：**群里是否真的收到了测试消息**（需用户确认）
4. 阶段 8 的 `systemctl list-timers douyu-watch.timer` 输出
5. 任何**跳过、失败或你不确定**的地方 —— 直接说，不要掩盖
